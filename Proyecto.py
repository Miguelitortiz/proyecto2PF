...
def login(user, password) -> int:
    '''
    Autentica un usuario basado en credenciales y devuelve el código de rol del usuario.

    Parámetros:
    - user (str): Nombre de usuario o número de cuenta.
    - password (str): Contraseña del usuario.

    Retorna:
    - int: Código que indica el rol del usuario (0 para error, 1 para admin, 2 para maestro, 3 para alumno).
    - clase_admin.Admin / clase_maestro.Maestro / clase_alumno.Alumno: Instancia del usuario autenticado.
    '''
...
def modificar_asistencia(materia_nombre: str, dia: int, mes: int, año: int, asistencia: bool) -> bool:
    '''
    Modifica la asistencia de una materia en una fecha específica.
    ...
    - asistencia (bool): Estado de asistencia (True para presente, False para ausente).

    Retorna:
    - bool: True si la asistencia se modificó correctamente, False en caso de error.
    '''
...
def horario_clase():
    ...
    '''Muestra el horario del usuario en una tabla, con opción de generar un PDF.
    '''
...
def asistencia_maestro():
    '''
Genera una tabla de asistencias de un maestro y permite la descarga en PDF.
'''
...
def id_maestro():
    ...
    '''    
Retorna el nombre de la materia asociada al maestro en función de su ID.

Retorna:
- str: Nombre de la materia asignada al maestro.
'''
...
def generar_pdf(dataframe, nombre_archivo):
    '''
Genera un PDF de una tabla de datos y guarda el archivo en el sistema.

Parámetros:
- dataframe (pd.DataFrame): Datos de la tabla.
